...
local BuyRemote = ReplicatedStorage.Remotes.Buy
local GunBuyRemote = ReplicatedStorage.Remotes.GunBuy

local ChangeSettingRemote = ReplicatedStorage.Remotes.ChangeSetting
local StorageRemote = ReplicatedStorage.Remotes.Storage
local PhoneRemote = ReplicatedStorage.Remotes.Phone

local SlideRemote = ReplicatedStorage.Modules.GunFramework.Remotes.Slide
local TickedRemote = ReplicatedStorage.Modules.GunFramework.Remotes.Ticked

-- ══════════════════════════════════════════════════════════════
-- SAFE STORAGE
-- ══════════════════════════════════════════════════════════════

secLabel(pageExt,"Safe Storage")

Config.TempSelectedItem = Config.TempSelectedItem or "None"
...
local function getInventoryItems()
    ...
    return (#items > 0 and items or {"None"})
end
...
do
    ...
    storeBtn.Size=UDim2.new(1,0,0,ROW_H)
    storeBtn.BackgroundColor3=Color3.fromRGB(0,30,50)
    storeBtn.BorderSizePixel=0
    storeBtn.Text="📦 Store Selected"
    storeBtn.TextColor3=C_ACCENT
    storeBtn.Font=Enum.Font.GothamBold
    storeBtn.TextSize=TXT_SIZE
    storeBtn.Parent=pageExt
    stroke(storeBtn,C_ACCENT,1)

    storeBtn.MouseButton1Click:Connect(function()
        local item = Config.TempSelectedItem

        if item and item ~= "None" then
            StorageRemote:FireServer("Store", item)

            storeBtn.Text = "✅ Stored"
...
do
    ...
    grabBtn.Size=UDim2.new(1,0,0,ROW_H)
    grabBtn.BackgroundColor3=Color3.fromRGB(0,30,50)
    grabBtn.BorderSizePixel=0
    grabBtn.Text="📤 Grab Selected"
    grabBtn.TextColor3=C_ACCENT
    grabBtn.Font=Enum.Font.GothamBold
    grabBtn.TextSize=TXT_SIZE
    grabBtn.Parent=pageExt
    stroke(grabBtn,C_ACCENT,1)
...
secLabel(pageExt,"Gun Shop")

Config.SelectedGun = Config.SelectedGun or "Glock17"

makeDropdown(pageExt,"Weapon","SelectedGun",{
    "Ruger",
    "Makarov",
    "Glock17",
    "M&P9",
...
do
    ...
    buyBtn.Size=UDim2.new(1,0,0,ROW_H)
    buyBtn.BackgroundColor3=Color3.fromRGB(0,30,50)
    buyBtn.BorderSizePixel=0
    buyBtn.Text="🔫 Buy Weapon"
    buyBtn.TextColor3=C_ACCENT
    buyBtn.Font=Enum.Font.GothamBold
    buyBtn.TextSize=TXT_SIZE
    buyBtn.Parent=pageExt
    stroke(buyBtn,C_ACCENT,1)

    buyBtn.MouseButton1Click:Connect(function()
        GunBuyRemote:FireServer(Config.SelectedGun)
...
Config.SelectedAmmo = Config.SelectedAmmo or "Pistol Ammo"

makeDropdown(pageExt,"Ammo","SelectedAmmo",{
    "Pistol Ammo",
    "Rifle Ammo",
    "SMG Ammo",
    "Shotgun Ammo"
...
do
    ...
    ammoBtn.Size=UDim2.new(1,0,0,ROW_H)
    ammoBtn.BackgroundColor3=Color3.fromRGB(0,30,50)
    ammoBtn.BorderSizePixel=0
    ammoBtn.Text="📦 Buy Ammo"
    ammoBtn.TextColor3=C_ACCENT
    ammoBtn.Font=Enum.Font.GothamBold
    ammoBtn.TextSize=TXT_SIZE
    ammoBtn.Parent=pageExt
    stroke(ammoBtn,C_ACCENT,1)

    ammoBtn.MouseButton1Click:Connect(function()
        GunBuyRemote:FireServer(Config.SelectedAmmo)

        ammoBtn.Text = "✅ Purchased"
