...
local DefaultConfig = {
    ...
    FlyEnabled=false, FlySpeed=50, RageMode=false,
    Whitelist={},
}
local Config = {}
...
-- GUI
-- ══════════════════════════════════════════════════════════════
local old=playerGui:FindFirstChild("NexusSystemUI"); if old then old:Destroy() end

local gui=Instance.new("ScreenGui")
gui.Name="NexusSystemUI"; gui.ResetOnSpawn=false; gui.IgnoreGuiInset=true
...
-- ── PANEL  (100% rectangular, sin UICorner en ningún lado) ───
local panelW,panelH=314,480
local main=Instance.new("Frame")
main.Name="NexusPanel"; main.Size=UDim2.fromOffset(panelW,panelH)
...
titleLbl.Size=UDim2.new(1,-100,0,24); titleLbl.Position=UDim2.fromOffset(12,6)
titleLbl.BackgroundTransparency=1; titleLbl.Text="⬡ NEXUS"
titleLbl.TextColor3=C_ACCENT; titleLbl.Font=Enum.Font.GothamBlack
titleLbl.TextSize=18; titleLbl.TextXAlignment=Enum.TextXAlignment.Left; titleLbl.Parent=header

local subLbl=Instance.new("TextLabel")
subLbl.Size=UDim2.new(1,-100,0,13); subLbl.Position=UDim2.fromOffset(13,27)
...
subLbl.TextSize=9; subLbl.TextXAlignment=Enum.TextXAlignment.Left; subLbl.Parent=header

-- botón X
local closeBtn=Instance.new("TextButton")
closeBtn.Size=UDim2.fromOffset(34,34); closeBtn.Position=UDim2.new(1,-38,0.5,-17)
...
closeBtn.Font=Enum.Font.GothamBold; closeBtn.TextSize=13; closeBtn.AutoButtonColor=false; closeBtn.Parent=header
...
local function makeTabPage()
    ...
    pad.PaddingRight=UDim.new(0,8); pad.PaddingBottom=UDim.new(0,8); pad.Parent=page
    local lay=Instance.new("UIListLayout")
    lay.SortOrder=Enum.SortOrder.LayoutOrder; lay.Padding=UDim.new(0,4); lay.Parent=page
    return page
end
...
local function secLabel(page,text)
    local f=Instance.new("Frame"); f.Size=UDim2.new(1,0,0,16); f.BackgroundTransparency=1; f.Parent=page
    ...
    l.TextSize=9; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=f
    return f
end
...
local function makeToggle(page,text,key,cb)
    ...
    lbl.Size=UDim2.new(1,-48,1,0); lbl.Position=UDim2.fromOffset(8,0)
    ...
    tog.Size=UDim2.fromOffset(36,18); tog.Position=UDim2.new(1,-42,0.5,-9)
    ...
    dot.Size=UDim2.fromOffset(14,14); dot.BorderSizePixel=0
...
local function makeSlider(page,text,key,mn,mx,cb)
    ...
    local lbl=Instance.new("TextLabel")
    lbl.Size=UDim2.new(1,-8,0,17); lbl.Position=UDim2.fromOffset(8,4)
    lbl.BackgroundTransparency=1; lbl.Text=text..": "..tostring(Config[key])
    lbl.TextColor3=C_TEXT; lbl.Font=Enum.Font.GothamMedium
    lbl.TextSize=11; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Parent=row

    local track=Instance.new("Frame")
    track.Size=UDim2.new(1,-16,0,5); track.Position=UDim2.fromOffset(8,26)
...
local function makeColorRow(page,text,rk,gk,bk)
    ...
    prev.Size=UDim2.fromOffset(22,22); prev.Position=UDim2.new(1,-26,0.5,-11)
...
local function makeDropdown(page,text,key,options)
    ...
    container.Size=UDim2.new(1,0,0,30); container.BackgroundTransparency=1
    ...
    for idx,opt in ipairs(options) do
        ...
        ob.Size=UDim2.new(1,0,0,26); ob.Position=UDim2.fromOffset(0,(idx-1)*26)
        ...
        ob.MouseButton1Click:Connect(function()
            ...
            open=false; container.Size=UDim2.new(1,0,0,30); arr.Text="▾"
            saveConfig()
        end)
    end
    ...
    hitbox.MouseButton1Click:Connect(function()
        ...
        if open then
            ...
            TweenService:Create(dropFrame,TWEENI,{Size=UDim2.new(1,0,0,#options*26)}):Play()
            container.Size=UDim2.new(1,0,0,30+#options*26)
        else
            TweenService:Create(dropFrame,TWEENI,{Size=UDim2.new(1,0,0,0)}):Play()
            task.delay(0.18,function() dropFrame.Visible=false end)
...
do
    ...
    inputRow.Size=UDim2.new(1,0,0,30); inputRow.BackgroundColor3=C_ROW
    inputRow.BorderSizePixel=0; inputRow.Parent=pageSet; stroke(inputRow,Color3.fromRGB(0,60,90),1)

    local nameBox=Instance.new("TextBox")
    nameBox.Size=UDim2.new(1,-70,1,-8); nameBox.Position=UDim2.fromOffset(5,4)
    ...
    nameBox.Font=Enum.Font.GothamMedium; nameBox.TextSize=11
    nameBox.ClearTextOnFocus=false; nameBox.Parent=inputRow
    stroke(nameBox,Color3.fromRGB(0,60,90),1)

    local addBtn=Instance.new("TextButton")
    addBtn.Size=UDim2.fromOffset(56,22); addBtn.Position=UDim2.new(1,-60,0.5,-11)
    addBtn.BackgroundColor3=Color3.fromRGB(0,30,50); addBtn.BorderSizePixel=0
    addBtn.Text="+ Add"; addBtn.TextColor3=C_ACCENT
    addBtn.Font=Enum.Font.GothamBold; addBtn.TextSize=10; addBtn.AutoButtonColor=false; addBtn.Parent=inputRow
    ...
    local function rebuildWL()
        ...
        for _,name in ipairs(Config.Whitelist) do
            ...
            e.Size=UDim2.new(1,0,0,26); e.BackgroundColor3=C_ROW
            e.BorderSizePixel=0; e.Parent=wlFrame; stroke(e,Color3.fromRGB(0,60,90),1)
            local nl=Instance.new("TextLabel")
            nl.Size=UDim2.new(1,-34,1,0); nl.Position=UDim2.fromOffset(6,0)
            ...
            db.Size=UDim2.fromOffset(24,18); db.Position=UDim2.new(1,-28,0.5,-9)
            db.BackgroundColor3=Color3.fromRGB(40,8,8); db.BorderSizePixel=0
            db.Text="✕"; db.TextColor3=Color3.fromRGB(255,80,80)
            db.Font=Enum.Font.GothamBold; db.TextSize=9; db.AutoButtonColor=false; db.Parent=e
...
do
    ...
    saveBtn.Size=UDim2.new(1,0,0,30); saveBtn.BackgroundColor3=Color3.fromRGB(0,30,50)
    saveBtn.BorderSizePixel=0; saveBtn.Text="💾  Guardar Config"
    saveBtn.TextColor3=C_ACCENT; saveBtn.Font=Enum.Font.GothamBold
    saveBtn.TextSize=11; saveBtn.AutoButtonColor=false; saveBtn.Parent=pageSet
...
do
    ...
    rstBtn.Size=UDim2.new(1,0,0,30); rstBtn.BackgroundColor3=Color3.fromRGB(35,8,8)
    ...
    rstBtn.TextSize=11; rstBtn.AutoButtonColor=false; rstBtn.Parent=pageSet
...
fab.Name="NexusFAB"; fab.Size=UDim2.fromOffset(48,48)
fab.Position=UDim2.new(1,-60,0.5,-24)
fab.BackgroundColor3=C_DARK; fab.BorderSizePixel=0
fab.AutoButtonColor=false; fab.Text="⬡"
fab.TextColor3=C_ACCENT; fab.Font=Enum.Font.GothamBlack
fab.TextSize=22; fab.ZIndex=20; fab.Parent=gui
        ...
        or inp.UserInputType==Enum.UserInputType.Touch then
            ...
            task.delay(0.4,function()
                if holding and not fabMoved then
                    ...
                    TweenService:Create(fab,TweenInfo.new(0.08),{TextColor3=Config.SilentAimEnabled and Color3.fromRGB(80,255,160) or C_ACCENT}):Play()
                end
            end)
        end
    end)
    UserInputService.InputChanged:Connect(function(inp)
        ...
        or inp.UserInputType==Enum.UserInputType.Touch) then
            ...
            if fabMoved then
                ...
                local nx=math.clamp(fabStartPos.X.Offset+delta.X,4,sc.X-52)
                local ny=math.clamp(fabStartPos.Y.Offset+delta.Y,4,sc.Y-52)
...
local function stopFly()
    ...
    if root then
        local bp=root:FindFirstChild("NexusFlyBP"); local bg=root:FindFirstChild("NexusFlyBG")
...
local function startFly()
    ...
    if not root:FindFirstChild("NexusFlyBP") then
        local bp=Instance.new("BodyPosition"); bp.Name="NexusFlyBP"
    ...
    if not root:FindFirstChild("NexusFlyBG") then
        local bg=Instance.new("BodyGyro"); bg.Name="NexusFlyBG"
...
print("[NEXUS v5.1] OK — "..player.Name)